      ...
      <AnimatePresence>
        {isTooltipVisible && !isDismissed && (
          <motion.div
            ...
            className="relative max-w-[260px] p-4 rounded-2xl rounded-br-md bg-card/95 backdrop-blur-xl border border-border/60 shadow-2xl"
          >
            {/* Close btn */}
            <button
              onClick={() => setIsDismissed(true)}
              className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-card border border-border/60 flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-primary/40 transition-all duration-200 shadow-lg"
              aria-label="Close"
            >
              <X className="w-3 h-3" />
            </button>
            ...
            <div className="flex items-start gap-3">
              <div className="w-9 h-9 rounded-full bg-[#25D366]/15 border border-[#25D366]/30 flex items-center justify-center shrink-0">
                <svg
                  viewBox="0 0 24 24"
                  className="w-4.5 h-4.5 text-[#25D366]"
                  fill="currentColor"
                >
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
                  Have questions? We're available on WhatsApp for quick support.
                </p>
              </div>
            </div>
          </motion.div>
